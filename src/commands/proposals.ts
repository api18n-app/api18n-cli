...
function buildClient() {
  ...
  return loadConfig(process.cwd()).then(
    (config) =>
      new Api18nClient({
        baseUrl: BACKEND_URL,
...
export async function runProposalsList(
  ...
  if (proposals.length === 0) {
    console.log(
      kleur.gray(status ? `No ${status} proposals.` : "No proposals."),
    );
    return;
  }
  ...
  for (const p of proposals) {
    ...
    if (p.summary) {
      console.log(
        `         ${kleur.gray("“" + truncate(p.summary, 80) + "”")}`,
      );
    }
  }
}
...
export async function runProposalsShow(
  ...
  console.log(
    `  ${kleur.gray("by")}     ${describeProposer(proposal.proposer)}`,
  );
