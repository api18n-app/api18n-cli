...
export async function runLogin(options: LoginOptions = {}): Promise<void> {
  ...
  const client = new Api18nClient({ baseUrl, token });
  let me;
  try {
    me = await client.me();
  } catch (err) {
    if (err instanceof ApiError) {
      console.error(kleur.red(`Sign-in failed (${err.status}): ${err.message}`));
    } else {
      console.error(kleur.red(`Sign-in failed: ${err instanceof Error ? err.message : String(err)}`));
    }
    process.exit(1);
  }
  ...
  const name =
    ...
    me.user.id;
  console.log();
  console.log(kleur.green('✓'), `Signed in as ${kleur.bold(name)} (${me.company.name})`);
}
